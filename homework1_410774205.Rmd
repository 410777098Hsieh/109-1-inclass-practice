...
```{r ans13}
# G5TimeParsed
G5TimeParsed <-paste0(Game5Time[[1]],"/",Game5Time[[2]],Game5Time[[3]])
print(G5TimeParsed)
```
...
```{r ans14}
# TpeJason
AllenTime <- ymd_hm("2020-10-09 21:00",
                    tz="America/New_York")
TpeJason <- with_tz(AllenTime, tzone = "Asia/Taipei")
print(AllenTime)
print(TpeJason)
...
```{r ans15}
# ParisLouis
ParisLouis0 <- AllenTime + minutes(15)
ParisLouis <- with_tz(ParisLouis0, tzone = "Europe/Paris")
print(ParisLouis)
...
```{r ans21}
# sumChNumbers
chNumbers_f <- factor(chNumbers,
                      levels = c("壹","貳","參","肆","伍","陸","柒","捌","玖"))
as.numeric(chNumbers_f) -> chNumbersn
sumchNumbers <- sum(chNumbersn)
print(sumchNumbers)
```
...
```{r ans22}
# sumChNumbersWith0
chNumbersWith0 <- factor(chNumbersWith0)
as.numeric(chNumbersWith0) -> chNumbersWith0
...
download.file(url="https://www.dropbox.com/s/rykdkmz27o1t4dh/IMG_2027.JPG?dl=1",
              destfile="hw1.jpg")
install.packages("magick")#安裝magick套件
install.packages("exiftoolr")#安裝exiftoolr套件

library(magick) # 處理圖片的套件
location1 <- image_read("hw1.jpg")
location1
library(exiftoolr) # 讀取圖片詳細資訊的套件
install_exiftool()#安裝涵式
