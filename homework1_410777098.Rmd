...
G5TimeParsed <- mdy_hm("Oct. 9 2020 21:00", tz="America/New_York")
print(G5TimeParsed)
class(G5TimeParsed)
```
...
# ParisLouis
ParisLouis <- 
  with_tz(G5TimeParsed+minutes(x=15),
          tzone="Europe/Paris")
ParisLouis
```
...
```{r ans21}
# sumChNumbers
chNumbers_factor <- factor(chNumbers,
         levels=c("壹","貳","參","肆","伍","陸","柒","捌","玖"))

sumChNumbers <- sum(as.numeric(chNumbers_factor))
...
```{r ans22}
# sumChNumbersWith0
chNumbersWith0_factor <- factor(chNumbersWith0, 
         levels=c("零","壹","貳","參","肆","伍","陸","柒","捌","玖"))

sumChNumbersWith0 <- sum(as.numeric(chNumbersWith0_factor))-length(chNumbersWith0_factor)
