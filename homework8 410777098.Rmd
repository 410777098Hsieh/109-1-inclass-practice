...
```{r ans122, echo=TRUE}
lowerTriX <- vector("list")


    for (i in 1:7) {
      for (j in 1:7) {
        if(i>=j)
            lowerTriX <- c(lowerTriX,sampleMatrix[i,j])
...
for (x in 1:length(districtSet)) {
  meanIncomes[[x]]<-mean(simData$income[str_which(simData$district,districtSet[[x]])])
}
meanIncomes
# meanIncomes
```
